...
lines=$(wc -l < "${COORDFILE}")
echo NUMBER OF LINES IS = ${lines}
scriptdir=$(dirname "$0")
coordbase=$(basename "${COORDFILE}")

#parallel -k -j ${CORES} --eta "${scriptdir}"/CalcWPSforCoordinate.sh --window ${WINDOW} --min ${MIN} --max 
#${MAX} -i "${BAMFILE}" --region {} :::: "${COORDFILE}" >> 
#"${BAMFILE}"_"${coordbase}"_"${WINDOW}"WPS_"${MIN}"-"${MAX}"bp

parallel --will-cite -k -j "${CORES}" --eta \
  "${scriptdir}/CalcWPSforCoordinate.sh" --window "${WINDOW}" --min "${MIN}" --max "${MAX}" \
  -i "${BAMFILE}" --region {} :::: "${COORDFILE}" \
  >> "${BAMFILE}_${coordbase}_${WINDOW}WPS_${MIN}-${MAX}bp"
